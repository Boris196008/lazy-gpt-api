...
def handle_request(data, first):
    ...
    try:
        ...
        if not (isinstance(action, str) and (action.startswith("custom:") or action.startswith("http"))):
            followup_prompt = (
                "Ответь на запрос пользователя, учитывая, что это может быть уточнение к предыдущей теме. "
                "Но обязательно выдай **свежую и сфокусированную** информацию, соответствующую новой формулировке. "
                "Не повторяй предыдущий ответ. "
                "Структура: законченный, читабельный, лаконичный текст. "
                "Если это follow-up о гастрономии — сосредоточься только на еде, ресторанах, рынках, а не на маршрутах или достопримечательностях."
            )

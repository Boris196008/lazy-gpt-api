...
def handle_request(data, first):
    ...
    elif not first and not isinstance(action, str):
        system_prompt = (
            "Ты — гениальный помощник. Это уточнение к предыдущему вопросу, но ты не должен повторять предыдущий ответ. "
            "Сконцентрируйся на новой теме. Не пересказывай уже сказанное. Ответ должен быть кратким, умным, точным и завершённым."
        )
    else:
        system_prompt = (

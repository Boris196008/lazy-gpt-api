    ...
    try:
        ...
        if not (isinstance(action, str) and (action.startswith("custom:") or action.startswith("http"))):
            followup_prompt = (
                ...
                "Не повторяй уже сказанное, не уточняй детали. "
                "⚠️ Важно: "
                "- Ответ должен быть только JSON-массивом, без текста вокруг. "
                "- Пример: [{\"label\": \"...\", \"action\": \"...\"}] "
                "- Все action — только внутренние: не начинаются с http:// или https:// и не содержат .com, .ru и т.п. "
                "- Не предлагай кнопки вроде \"Завершить\" или \"Спасибо\" — только полезные продолжения. "
                "Ты всегда должен предложить follow-up, даже если кажется, что ответ исчерпывающий."
            )
